...
from modules.analyzer import calcular_compatibilidad, evaluar_habilidades, extraer_habilidades
...
class RecruitAITestCase(unittest.TestCase):
    ...
    def test_civil_engineering_skills_are_detected_from_vacancy_terms(self):
        vacancy = {
            "habilidades": "AutoCAD, Civil 3D, MS Project, Excel, Gestión de Obras, Metrados, Valorizaciones",
            "experiencia": 0,
        ...
        text = (
            "Ingeniero civil bachiller con manejo de Microsoft Excel, AutoCAD Civil 3D, "
            "S10, MS Project, metrados y valorizaciones en obras civiles."
        )
        skills = extraer_habilidades(text, vacancy["habilidades"].split(", "))
        self.assertIn("AutoCAD", skills)
        self.assertIn("Civil 3D", skills)
        self.assertIn("MS Project", skills)
        self.assertIn("Gestión de Obras", skills)
        self.assertGreaterEqual(calcular_compatibilidad(text, vacancy), 85)

    def test_semantic_skill_detection_uses_vacancy_terms_without_catalog_changes(self):
        vacancy = {
            ...
            "experiencia": 0,
            "estudios": "",
            "peso_habilidades": 100,
            "peso_experiencia": 0,
            "peso_estudios": 0,
        }
        text = "Experiencia brindando servicio al cliente, resolviendo consultas y registrando pagos en caja."
        with patch("modules.analyzer.detectar_habilidades_semanticas", return_value=["Atención al cliente"]):
            skills, missing = evaluar_habilidades(text, vacancy)
        self.assertIn("Atención al cliente", skills)
        self.assertEqual(missing, ["Manejo de caja"])
        self.assertEqual(calcular_compatibilidad(text, vacancy, skills, missing), 50.0)
